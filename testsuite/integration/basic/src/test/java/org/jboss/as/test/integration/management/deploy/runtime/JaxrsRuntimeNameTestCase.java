...
public class JaxrsRuntimeNameTestCase extends AbstractRuntimeTestCase {
    ...
    public void testStepByStep() throws Exception {
        ...
        for (ModelNode jaxrsResource: jaxrsResources) {
            if (jaxrsResource.get(RESOURCE_CLASS.getName()).asString().equals(HelloResource.class.getName())) {
                count++;
                String path = jaxrsResource.get(RESOURCE_PATH.getName()).asString();
                switch (path) {
                    case "/update":
                        assertThat(jaxrsResource.toString(), jaxrsResource.get(RESOURCE_METHODS.getName()).asList().get(0).asString(), is("PUT /hello-rs/hello/update - org.jboss.as.test.integration.management.deploy.runtime.jaxrs.HelloResource.updateMessage(...)"));
                        break;
                    case "/json":
                        assertThat(jaxrsResource.toString(), jaxrsResource.get(RESOURCE_METHODS.getName()).asList().get(0).asString(), is("GET /hello-rs/hello/json - org.jboss.as.test.integration.management.deploy.runtime.jaxrs.HelloResource.getHelloWorldJSON()"));
                        break;
                    case "/xml":
                        assertThat(jaxrsResource.toString(), jaxrsResource.get(RESOURCE_METHODS.getName()).asList().get(0).asString(), is("GET /hello-rs/hello/xml - org.jboss.as.test.integration.management.deploy.runtime.jaxrs.HelloResource.getHelloWorldXML()"));
                        break;
                    case "/":
                        assertThat(jaxrsResource.toString(), jaxrsResource.get(RESOURCE_METHODS.getName()).asList().get(0).asString(), is("GET /hello-rs/hello/ - org.jboss.as.test.integration.management.deploy.runtime.jaxrs.HelloResource.getHelloWorld()"));
                        break;
                    default:
                        assertThat(jaxrsResource.toString(), false, is(true));
                }
            } else if (jaxrsResource.get(RESOURCE_CLASS.getName()).asString().equals(PureProxyApiService.class.getName())) {
                count++;
                String path = jaxrsResource.get(RESOURCE_PATH.getName()).asString();
                assertThat(jaxrsResource.toString(), path, is("pure/proxy/test/{a}/{b}"));
                assertThat(jaxrsResource.toString(), jaxrsResource.get(RESOURCE_METHODS.getName()).asList().get(0).asString(), is("GET /hello-rs/hello/pure/proxy/test/{a}/{b} - org.jboss.as.test.integration.management.deploy.runtime.jaxrs.PureProxyApiService.test(...)"));
            }
        }
    ...
    public void testReadRestEndPointIntf() throws Exception {
        ...
        assertThat(res.get(RESOURCE_CLASS.getName()).asString(), is(PureProxyEndPoint.class.getCanonicalName()));
        List<ModelNode> subResList = res.get(RESOURCE_PATHS.getName()).asList();
        assertThat(subResList.size(), is(1));

        ModelNode rootRes = subResList.get(0);
        assertThat(rootRes.get(RESOURCE_PATH.getName()).asString(), is("pure/proxy/test/{a}/{b}"));
        assertThat(rootRes.get(JAVA_METHOD.getName()).asString(), is("java.lang.String " + PureProxyEndPoint.class.getCanonicalName() + ".test(@PathParam java.lang.String a, @PathParam java.lang.String b)"));
        assertThat(rootRes.get(CONSUMES.getName()).isDefined(), is(false));
        assertThat(rootRes.get(PRODUCES.getName()).isDefined(), is(false));
        assertThat(rootRes.get(RESOURCE_METHODS.getName()).asList().size(), is(1));
        assertThat(rootRes.get(RESOURCE_METHODS.getName()).asList().get(0).asString(), is("GET /hello-rs/hello/pure/proxy/test/{a}/{b}"));
